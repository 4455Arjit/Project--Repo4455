...
app.get("/all",(requ,resp)=>{        //Displaying all customers on one page-> DONE✅
    console.log(api_cards);
    resp.json(api_cards);
});
app.get("/specific_customer/:phone",(requ,resp)=>{     //Displaying a specific Customer using Phone-Number-> DONE✅
    const get_customer=api_cards.filter((api_inst)=>api_inst.phone_no==requ.params.phone);
...
app.post("/addcustomer",(requ,resp)=>{    // Adding a NEW Customer-> DONE✅
...
app.patch("/specific_customer/:phone",(requ,resp)=>{
    const find_cust=api_cards.find((api_inst)=>api_inst.phone_no==requ.params.phone);
        find_cust.person_name=requ.body["custmr_name"]||find_cust.person_name;
        find_cust.payment_status=requ.body["payment_status"]||find_cust.payment_status ;
        find_cust.reason_for_earning=requ.body["job_i_did"]||find_cust.reason_for_earning ;
        find_cust.deadline_for_payment=requ.body["payment_deadline"]||find_cust.deadline_for_payment;

    resp.json(find_cust);
});
app.delete("/delete_customer/:phone",(requ,resp)=>{
    const find_cust=api_cards.findIndex((api_inst)=>api_inst.phone_no==requ.params.phone);
    console.log(find_cust);
    const removing=api_cards.splice(find_cust,1)[0];
    resp.json(removing);
});
app.listen(port,()=>{
