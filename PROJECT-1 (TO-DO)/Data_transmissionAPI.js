...
appl.get("/api/all_tasks",async(requ,resp)=>{     // "/api/all_tasks" route in API
    let full_info=await datab.query("SELECT * FROM task_crush");
...
appl.post("/api/adding_user_task",async (requ,resp)=>{  // "/adding_user_task" route in API ->adding new Task
    ...
    try{
        await datab.query("INSERT INTO task_crush (task_info) VALUES($1)",[user_task]);
...
appl.get("/api/edit_task/:id",async(requ,resp)=>{
    let edit_task=await datab.query("SELECT * FROM task_crush where id=$1",[requ.params.id]);
...
appl.post("/api/editing_task/:id",async(requ,resp)=>{
    try{
        ...
        await datab.query("UPDATE task_crush SET task_info=$1 where id=$2",[user_taskinp,requ.params.id]);
        resp.redirect("/api/all_tasks");
    }catch(er){
        resp.send(er);
        console.log(er);
...
appl.post("/api/deleting_task/:id",async(requ,resp)=>{
    try{
        ...
        let del_q=await datab.query("DELETE FROM task_crush WHERE id=$1 RETURNING*",[deletion_task]);
